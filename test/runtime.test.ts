...
describe("runtime", () => {
    ...
    globalThis.fetch = mock(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/backend-api/wham/usage")) {
        ...
        return new Response(JSON.stringify({
          rate_limit: {
            primary_window: { used_percent: primary },
  ...
  it("switches to another saved account when the live usage probe is unavailable", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "multi-auth-runtime-"))
    const file = path.join(dir, "auth.json")
    process.env.OPENCODE_MULTI_AUTH_PATH = file
    await upsert(file, {
    ...
    expect(json).toEqual({ ok: true, auth: "Bearer a2-access", account: "acct-2" })
    expect((await active(file))?.id).toBe("a2")
    expect(sets.length).toBeGreaterThan(0)
